...
/// Pasteboard types that mean "do not record this copy".
///
/// `org.nspasteboard.ConcealedType` is the convention from nspasteboard.org
/// that password managers (1Password, Bitwarden, KeePassXC, Secretive, …)
/// put on the pasteboard alongside a copied secret precisely so that
/// clipboard managers skip it. The Transient/AutoGenerated markers are the
/// same convention for short-lived and machine-generated content, and the
/// last two are older vendor-specific spellings still emitted in the wild.
///
/// Honoring these is not optional for a clipboard manager — without it we
/// silently archive every password the user copies out of their vault.
...
/// Returns the privacy marker present on the general pasteboard, if any.
///
/// `None` means the pasteboard carries no "don't record me" hint, so the
/// copy is safe to persist. Any failure to read the pasteboard also yields
/// `None`: the watcher already has the content at that point, and refusing
/// to store on a transient AppKit hiccup would silently drop real history.
pub fn pasteboard_privacy_marker() -> Option<&'static str> {
    unsafe {
        ...
        for i in 0..count {
            ...
            if let Some(marker) = PRIVACY_MARKERS
                .iter()
                .find(|m| name.eq_ignore_ascii_case(m))
            {
                return Some(marker);
            }
        }
